; (function ($, window, undefined) {
    ...
    var myrDialog = {
        myopen: function (m) {
            if (myLoadding) myLoadding.close();
            myLoadding = $.dialog(m);
        },
        open: function () {
            if (myLoadding) myLoadding.close();
        ...
        openm: function (m) {
            if (myLoadding) myLoadding.close();
            myLoadding = $.dialog({ content: '' + m + '' });
        },
        close: function () {
            myLoadding.close();
        },
        alert: function (m) { if (myLoadding) myLoadding.close(); $.dialog({ content: '' + m + '', title: "alert", time: 3000 }); },
        alert_ok: function (m, fun) {
            if (myLoadding)
                myLoadding.close();
            var obj0 = $.dialog();

            $.dialog({ content: '' + m + '', title: "ok", time: 5000, ok: function () { fun(); } });
        },
        ok: function (m, fun) { if (myLoadding) myLoadding.close(); $.dialog({ content: '' + m + '', title: "ok", ok: function () { fun(); } }); }
    }
    $.mydialog = myrDialog;
