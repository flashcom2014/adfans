...
namespace WxPayAPI
{
    ...
    public class WxPayData
    {
       ...
       * @throws WxPayException
       */
        public SortedDictionary<string, object> FromXml(string xml,ref WxPayConfig wx)
        {
            if (string.IsNullOrEmpty(xml))
            {
                Log.Error(this.GetType().ToString(), "将空的xml串转换为WxPayData不合法!");
            ...
            /*try
            {
                if (Order.IsYsPay(m_values["out_trade_no"].ToString()))
                {
                    int yskj_payment_id = int.Parse(System.Configuration.ConfigurationManager.AppSettings["yskj_payment_id"].ToString());
                    wx = new WxPayConfig(yskj_payment_id);
                }
                else
                {
                    string mch_id = Order.GetMch_Id(m_values["out_trade_no"].ToString());
                    wx = new WxPayConfig(mch_id);
                }
                CheckSign(wx);//验证签名,不通过会抛异常
            }
        ...
        * @throws WxPayException
        */
        public SortedDictionary<string, object> FromXml(string xml, WxPayConfig wx)
        {
            if (string.IsNullOrEmpty(xml))
            {
                Log.Error(this.GetType().ToString(), "将空的xml串转换为WxPayData不合法!");
        ...
        */
        public string MakeSign(WxPayConfig wx)
        {
            //转url格式
            string str = ToUrl();
            //在string后加入API KEY
        ...
        * 正确返回true，错误抛异常
        */
        public bool CheckSign(WxPayConfig wx)
        {
            //如果没有设置签名，则跳过检测
            if (!IsSet("sign"))
            {
