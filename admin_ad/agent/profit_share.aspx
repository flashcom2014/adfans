<%@ Page Language="C#" AutoEventWireup="true" CodeFile="profit_share.aspx.cs" Inherits="profit_share" %>
                            ...
                            <tr>
                                 <td>
                                    <div style="width:100%; float:left; text-align:left; margin-left:20px;">
                                        ...
                                        &nbsp;
                                         &nbsp;
                                         <label id="appidlabel"  runat="server" >子广告商：</label>
    ...
    , PageType: 0
        ...
        var Getappid_Action = function () {
            ...
            $.ajax(
              ...
              {
                  type: "post",
                  url: "profit_share.aspx",
                  data: "action=getchild",
                  dataType: "json",
                  async: false,
                  success: function (json) {
                      ...
                      $.each(json.ListData, function (idx, item) {
                          $("#appid_list").append("<option value='" + item.ad_id + "'>" + item.ad_name + "(" + item.ad_mobile +")"+ "</option>");
                      })
                  },
                  error: function (data) {
                      $.mydialog.close();
...

              {
                  ...
                  success: function (json) {
                      ...
                      if (json.ListData.length == 0) {
                          $("#listiteam").append('<tr><td align="center" colspan="6" >无分成记录</td></tr>');
                          return;
                      }

                      $.each(json.ListData, function (idx, item) {
                          $("#listiteam").append("<tr><td>"+item.create_time+"</td><td>" + item.ad_name + "(" + item.ad_mobile + ")" + "</td><td>" + item.child_scale + "</td><td>" + item.actual_scale +"</td><td>" + item.amount/100 + "</td><td>" + item.money/100 +  "</td></tr>");
                      })
                  },
                  error: function (data) {
                      $.mydialog.close();
